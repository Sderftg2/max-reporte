...
class TelegramReporter:
    ...
    def report_channel(self):
        with self.client as client:
            ...
            # FIXED: Added proper error handling with return statement
            try:
                channel_entity = client.get_entity(self.channel_username)
            except Exception as e:
                print(f"{lrd}Username does not exist or error: {e}")
                return  # Exit function if username not found
            
            # Only continue if channel_entity was successfully created
            if self.method == "1":
